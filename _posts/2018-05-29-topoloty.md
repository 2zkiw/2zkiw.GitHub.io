...
## Proposition:
If $x_{n}_{n=1}^{\infty}$ is a sequence in R such that ($x_{n}$) -> x and ($x_{n}$) -> y then x = y. i.e. Limits of convergent sequences are unique.

- **Proof Method**: Triangle Inequality + $\fraction{\epsilon}{2}$ + max{N1, N2}

## Limit Laws for Sequences

Let($a_{n}$) -> L and ($b_{n}$) -> M be convergent sequences.
- The sequence ($a_{n}$ + $b_{n}$) is convergent and ($a_{n}$ + $b_{n}$) -> L + M
- The sequence ($a_{n}$ $b_{n}$) is convergent and ($a_{n}$ $b_{n}$) -> LM.
- For any $\alpha$ $\in$ R the sequence $(\alpha a_{n})$ converges and ($\alpha$ $a_{n}$) -> $\alpha$ L
- If M $\neq$ 0 then the sequence($\fraction{a_{n}{b_{n}}}$ converges and ($\fraction{a_{n}{b_{n}}}$) -> L/M

- **Common Proof Method for $\epsilon - \delta$**: Triangle Inequality + $\fraction{\epsilon}{2}$ + max{N1, N2}

## Squeeze Theorem for Sequences (in R)
Let ($a_{n}$), ($b_{n}$) and ($c_{n}$) be sequences in R, and assume that for sufficiently large k we have $a_{n} \leq b_{n} \leq c_{n}$. If ($a_{n}$) -> L and ($c_{n}$) -> L, then ($b_{n}$) is also convergent with limit L.
...
## Proposition:
If $(x_{n})_{n=1}^{\infty}$ -> x is a convergent sequence, then every subsequence ($x_{n_{k}}$) is also convergent with the same limit.(i.e. the same N works for $x_{n_{k}}$).

# Sequence in $R^n$

## Convergence:
Let $(x_{n})_{n=1}^{\infty}$ be a sequence in $R^{n}$. Say ($x_{n}$) converges with limit x $\in$ $R^{n}$, denoted ($x_{n}$) -> x, if for every $\epsilon$ > 0 there exists an N $\in$ N such that whenever n > N then ||$x_{n}$ - x|| < $\epsilon$.
...
## Proposition:
If S $\subseteq$ $R^{n}$, then x $\in$ $\bar{S}$ $\Longleftrightarrow$ **there exists** a convergent sequence ($x_{n}$) in S such that (x_{n}) -> x.

- Note: If S $\subseteq$ $R^n$, define ($x_{n}$) a sequence in S if $x_{n}$ $\in$ S for every n $\in$ N. 

- **Common proof method**: construct a sequence converges to x by construction $x_{n}$ $\in$ $B_{\fraction{1}{n}}$(x) and choose N s.t. $\fraction{1}{n}$ < $\epsilon$.
